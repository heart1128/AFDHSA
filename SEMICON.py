...
from models.cross_attention import Cross_Refine
from Encoder.Encoder import EncoderDefault
...
class ChannelTransformer(nn.Module):
    ...
    def forward(self, x):
        B, C, H, W = x.shape
        if Debug:
            print('ChannelTransformer.x.shape = ', x.shape) # torch.Size([16, 2048, 7, 7])
        ...
        if Debug:
            print('ChannelTransformer.out.shape = ', x.shape) # torch.Size([16, 2048, 7, 7])
        return x
...
class ResNet_Refine(nn.Module):
    ...
    def __init__(self, block, layer, is_local=True, num_classes=1000, zero_init_residual=False,
        ...
        if Debug:
            print('ResNet_Refine.INCO.layersNUms = ', layer) # 3
    ...
    def _forward_impl(self, x):
        if Debug:
            print('ResNet_Refine+ICON.input.shape = ', x.shape)  # torch.Size([16, 1024, 14, 14])
        x = self.layer4(x)
        if Debug:
            print('ResNet_Refine+ICON.layer4.out.shape = ', x.shape) # torch.Size([16, 2048, 7, 7])

        pool_x = self.avgpool(x)
        pool_x = torch.flatten(pool_x, 1)
        if self.is_local:
            if Debug:
                print('ResNet_Refine+ICON.local_out.x.shape = ', x.shape) # torch.Size([16, 2048, 7, 7])
                print('ResNet_Refine+ICON.local_out.pool_x.shape = ', pool_x.shape) # torch.Size([16, 2048])
            return x, pool_x
        else:
            if Debug:
                print('ResNet_Refine+ICON.gloabl_out.x.shape = ', x.shape)
            return x, pool_x
...
class SEM(nn.Module):
    ...
    def __init__(self, block, layer, att_size=4, num_classes=1000, zero_init_residual=False,
        ...
        self.input_dim = 64
        self.num = 3
        self.project_q = nn.Linear(self.input_dim, self.input_dim, bias=False)
        self.project_k = nn.Linear(14*14, self.input_dim, bias=False)

        self.feature = nn.Sequential(
            conv1x1(self.input_dim, 1),
            nn.BatchNorm2d(1),
    ...
    def _forward_impl(self, x):
        ...
        x = self.layer4(x)#bs*64*14*14 # resnet50拿出来的特征图
        
        B,C,H,W = x.size()
        origin_x = x
        x = x.view(B,C, -1) # [B,C,HW]

        q = self.project_q(x.permute(0, 2, 1)) # [B,HW,C]
        k = self.project_k(x) # [B,C,C]
        v = origin_x # [b,c,h,w]
        ...
        for _ in range(self.num):
            ...
            attn = self.feature(attn)
            
            
            attn = 2 - self._mask(attn, v)
            v = v.mul(attn.repeat(1, self.input_dim, 1, 1))

            features.append(self.feature(v))

            q = self.project_q(v.view(B,C,-1).permute(0, 2, 1)) # [B,HW,C]
            k = self.project_k(v.view(B,C,-1)) # [B,C,C]
...
class LatentEncoder(nn.Module):
    def __init__(self, in_channels , lod, blend, device):
        ...
        self.conv = nn.Sequential(
            ...
            nn.Conv2d(in_channels, in_channels, kernel_size=5, stride=1),
            nn.BatchNorm2d(in_channels),
        )

        self.conv1x1 = nn.Sequential(nn.Conv2d(1, 3, 1), nn.BatchNorm2d(3))

        self.encoder = EncoderDefault(startf=64,layer_count=7,maxf=256, latent_size=256, channels=3, device=device)
        state_dict = torch.load('./Encoder/Encoder.pth')
    ...
    def forward(self, x):
        B, _, _, _ = x.shape # 【16， 1024， 14， 14】
        x = self.conv1x1(self.conv(x).view(B, -1, 256, 256))
        x = self.encoder(x, self.lod, self.blend) # torch.Size([16, 1, 256])
        
        return x.view(B, -1) # [16, 256]
...
class SEMICON(nn.Module):
    def __init__(self, code_length=12, num_classes=200, att_size=3, feat_size=2048, device='cpu', pretrained=True):
        ...
        self.latentEncoder = LatentEncoder(feat_size // 2, lod=6, blend=1, device=device) # lod = 6 ->size==256
        ...
        if self.code_length != 32:
            self.W_G = nn.Parameter(torch.Tensor(code_length//2, feat_size // 8))
            torch.nn.init.kaiming_uniform_(self.W_G, a=math.sqrt(5))
        else:
            self.W_G = nn.Parameter(torch.Tensor(code_length//2+1, feat_size // 8))
            torch.nn.init.kaiming_uniform_(self.W_G, a=math.sqrt(5))
    ...
    def forward(self, x):
        ...
        # global_origin, global_f = self.refine_global(out)
        ...
        # print(out.shape) # torch.Size([16, 1024, 14, 14])
        latent_global = self.latentEncoder(out)
        # print(latent_global.shape) # torch.Size([16, 256])
        deep_S_G = F.linear(latent_global, self.W_G)
        # print(deep_S_G.shape) # torch.Size([16, 6])
