...
def train(query_dataloader, train_dataloader, retrieval_dataloader, code_length, args):
    ...
    if args.optim == 'SGD':
        # 过滤冻结的Encoder参数 
        optimizer = optim.SGD(filter(lambda p: p.requires_grad, model.parameters()), lr=args.lr, weight_decay=args.wd, momentum=args.momen, nesterov=True)
    ...
    start = time.time()
    best_mAP = 0
    for it in range(args.resume_iter, args.max_iter):
        ...
        for epoch in range(args.max_epoch):
            ...
            for batch, (data, targets, index) in pbar:
                ...
                # 新加mps
                sample_index = sample_index.to(F.device)
