...
def run(image_name=None, datasets=None, root=None, info=None, code_size=None):
    seed_everything(68)
    args = load_config()
    if args.arch != 'test' and args.arch != 'test_image':
    ...
    torch.backends.cudnn.benchmark = True
    print(args.arch)
    if args.arch == 'test_image':
        args.dataset = datasets
        args.root = root 
        args.info = info
        args.code_length = code_size
    ...
    elif args.arch == 'test_image':
        net_arch = retrieval_image
    ...
    if args.arch != 'test' and args.arch != 'test_image':
        ...
        for code_length in args.code_length:
            print("test_images start....")
            retrieval_image_path = net_arch.test(query_dataloader, train_dataloader, retrieval_dataloader, code_length, args, image_name)
            return retrieval_image_path
...
def load_config():
    ...
    parser.add_argument('--dataset', default='cub-2011',
                        help='Dataset name.')
    parser.add_argument('--root', default='../datasets/CUB_200_2011',
    ...
    parser.add_argument('--optim', default='SGD', type=str,
                        help='Optimizer')
    parser.add_argument('--code-length', default='32', type=str,
    ...
    parser.add_argument('--info', default='cub-200',
                        help='Train info')
    parser.add_argument('--arch', default='test_image',
                        help='Net arch')
